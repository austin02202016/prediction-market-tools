...
out.append(f"Which tool fits depends on how many venues you trade. If you trade one, the exchange's own terminal (Kalshi Pro, Polymarket's app) is usually enough. If you trade several, look at the cross-venue terminals in the first table, which merge books and route orders (Kairos and Stand are the ones executing across venues as of this check). Bots, alert services, analytics and the {len(by.get('Data & APIs',[]))} APIs and SDKs are each in their own section. Everything was re-checked on {today}.\n")
